	...
	for _, event := range pendingEvents {
		...
		case constants.EventTypeBookingCreated:
			...
			if err := json.Unmarshal(event.Payload, &bookingCreated); err != nil {
				log.Printf("[OutboxWorker] Error deserializing booking_created event %s: %v", event.ID, err)
		...
		case constants.EventTypeBookingCancelled:
			...
			if err := json.Unmarshal(event.Payload, &bookingCancelled); err != nil {
				log.Printf("[OutboxWorker] Error deserializing booking_cancelled event %s: %v", event.ID, err)
				_ = w.repo.MarkProcessed(ctx, event.ID)
				continue
			}
		...
		default:
			log.Printf("[OutboxWorker] Unknown event type: %s", event.EventType)
			_ = w.repo.MarkProcessed(ctx, event.ID)
			continue
		}

		if pubErr != nil {
			log.Printf("[OutboxWorker] Error publishing event %s: %v", event.ID, pubErr)
			continue
		}

		if err := w.repo.MarkProcessed(ctx, event.ID); err != nil {
			log.Printf("[OutboxWorker] Error marking event %s as processed: %v", event.ID, err)
		}
	}
}
